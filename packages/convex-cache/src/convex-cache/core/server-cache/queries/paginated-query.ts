...
export const _useCachedPaginatedQueryServer = <Q extends PQ_Query>({
  query,
  args,
  options,
  preloadedData,
  revalidateCache,
}: T_UseCachedPaginatedQueryServer<Q>): PQ_Result<Q> => {
  ...
  useEffect(() => {
    ...
    revalidateCache({
      query,
      args: { ...args, paginationOpts: { numItems: options.initialNumItems, cursor: null } } as PQ_ArgsPreloaded<Q>,
    });
  }, [raw]);
