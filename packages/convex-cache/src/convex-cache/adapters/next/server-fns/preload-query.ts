...
export const _preloadQuery = async ({
  queryName,
  args,
  options,
}: T_PreloadQueryParams<Q_Query>): Promise<Q_Result<Q_Query>> => {
  "use cache";
...
export const _preloadPaginatedQuery = async ({
  queryName,
  args,
  options,
}: T_PreloadPaginatedQueryParams<PQ_Query>): Promise<PQ_CachedResult<PQ_Query>> => {
  "use cache";
...
const convertPaginatedResultForClient = <Q extends PQ_Query>({
  result,
}: {
  result: Q_Result<Q>;
}): PQ_CachedResult<Q> => {
