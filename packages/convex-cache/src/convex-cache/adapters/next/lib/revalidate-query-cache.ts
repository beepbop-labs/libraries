...
export const revalidateQueryCache = async <Q extends Q_Query>({
  query,
  args,
}: {
  query: Q;
...
export const revalidatePaginatedQueryCache = async <Q extends PQ_Query>({
  query,
  args,
}: T_RevalidatePaginatedQueryCacheParams<Q>) => {
  const queryName = getFunctionName(query);
