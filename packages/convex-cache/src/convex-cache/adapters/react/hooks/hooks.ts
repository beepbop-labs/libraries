...
import type { Q_Query, PQ_Query, PQ_CachedResult, PQ_Item } from "../../../core/types";
import { type T_UseCachedQueryClient, _useCachedQueryClient } from "../../../core/client-cache/queries/query";
...
export const useCachedPaginatedQueryClient = <Q extends PQ_Query>({ query, args, options }: T_UsePaginatedQuery<Q>) => {
  ...
  return _useCachedPaginatedQueryClient({
    ...
    options,
    schemaMap,
    useLocalDb: (useLocalDb ?? useLocalDbDefault) as typeof useLocalDbDefault<PQ_CachedResult<Q>>,
  });
};
